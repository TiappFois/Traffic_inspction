...
import 'package:ti/commonutils/ti_utilities.dart';
import 'package:ti/model/SttnInspModels/safety_circular_bulletin_Model.dart';
import 'package:ti/commonutils/logger.dart';


class SafetyCircularBulletin extends StatefulWidget {
  @override
  _SafetyCircularBulletinState createState() => _SafetyCircularBulletinState();
}

class _SafetyCircularBulletinState extends State<SafetyCircularBulletin> {
  File imageFile = null ;
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('SafetyCircular');

  static SafetyCircularBulletinModel safetyCircularMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#26#')) {

      try {
        print("Safety Circular data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringsafetyCircular : = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getSaftyCrclrRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          safetyCircularMod = await new SafetyCircularBulletinModel(
              SttnInspDtlsList.getSafetyCircularList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of SafetyCircular");

      showText = await [false, false, false, false];

      safetyCircularMod = await new SafetyCircularBulletinModel(
          SttnInspDtlsList.getSafetyCircularList(), ['SELECT', 'SELECT', 'SELECT', 'SELECT'],
          ['', '', '', ''],
          '');
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          safetyCircularMod.ynList1[0] +
          safetyCircularMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar:
            TiUtilities.tiAppBar(context, "Safety circular / Bulletin File"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/First_Aid_Box_Stret"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < safetyCircularMod.ynList1.length ; i++){
                if(safetyCircularMod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  safetyCircularMod.ynList1[i] = '';
              ...
              if(selectedCount == 0){
                ...
                safetyCircularMod.base64Image = base64Image;

                safetyCircularMod.inspID = TiUtilities.inspmstr.inspid;

                if (_formKey.currentState.validate()) {
                  TiUtilities.callSttnInspEntryWebService(
                      context, json.encode(safetyCircularMod.toJson()),
                      "saveSaftyCrclrRgtr").then((res) {
                    if (res == 'Record Successfully Saved.') {
                      print('Record Successfully Saved.');
                      TiUtilities.showOKDialog(context, "Success!!")
                          .then((res1) {
                        Navigator.pushNamed(context, '/First_Aid_Box_Stret');
                      });
                    } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: safetyCircularMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getSafetyCircularListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: safetyCircularMod.sList1,
                      ynlst: safetyCircularMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: safetyCircularMod.whyNoRsn,
                      regiType: "safetycircularlang"),
                ),
              ),
            );
          },
      ...
      Expanded(
          ...
          child: Container(
              child: TextFormField(
                  ...
                  maxLines: 5,
                  onChanged: (val) {
                    safetyCircularMod.rmrks1 = val;
                    //print('safetyCircularMod.rmrks1:' + safetyCircularMod.rmrks1);
    })),
    )),
    new IconButton(
    onPressed: _listen,
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            safetyCircularMod.rmrks1= rmrkController.text;
  ...
  Future<String> _callSaveSafetyCircularBulletinWebService(
      SafetyCircularBulletinModel cautOrdReg) async {
    //log.d(alpa);
