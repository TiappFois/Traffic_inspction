...
import 'package:ti/model/SttnInspModels/station_working_rule_model.dart';
import 'package:flutter/material.dart';
import 'package:ti/commonutils/ti_utilities.dart';
import 'package:ti/commonutils/logger.dart';

class StationworkingRuleRegister extends StatefulWidget {
  @override
  _StationworkingRuleRegisterState createState() =>
      _StationworkingRuleRegisterState();
}

class _StationworkingRuleRegisterState
    extends State<StationworkingRuleRegister> {
  File imageFile = null ;
  ...
  static StationworkingRuleRegisterModel sttnwrkngrulemod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#21#')) {

      try {
        print("Attnd Reg data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringAttndReg = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getSttnWrkgRuleRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          sttnwrkngrulemod = await new StationworkingRuleRegisterModel(
              SttnInspDtlsList.getSttnWrkngRuleList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Attend reg");

      showText = await [false, false, false];

      sttnwrkngrulemod = await new StationworkingRuleRegisterModel(
          SttnInspDtlsList.getSttnWrkngRuleList(), ['SELECT', 'SELECT','SELECT'],
          ['', '',''],
          '');
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          sttnwrkngrulemod.ynList1[0] +
          sttnwrkngrulemod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Station Working Rule"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/sttn_mstr_diary"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < sttnwrkngrulemod.ynList1.length ; i++){
                if(sttnwrkngrulemod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  sttnwrkngrulemod.ynList1[i] = '';
              ...
              if(selectedCount == 0){
                ...
                sttnwrkngrulemod.base64Image = base64Image;

                sttnwrkngrulemod.inspID = TiUtilities.inspmstr.inspid;

                if (_formKey.currentState.validate()) {
                  TiUtilities.callSttnInspEntryWebService(
                      context, json.encode(sttnwrkngrulemod.toJson()),
                      "saveSttnWrkgRuleRgtr").then((res) {
                    if (res == 'Record Successfully Saved.') {
                      print('Record Successfully Saved.');
                      TiUtilities.showOKDialog(context, "Success!!")
                          .then((res1) {
                        Navigator.pushNamed(context, '/sttn_mstr_diary');
                      });
                    } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: sttnwrkngrulemod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getSttnWrkngRuleListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: sttnwrkngrulemod.sList1,
                      ynlst: sttnwrkngrulemod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: sttnwrkngrulemod.whyNoRsn,
                      regiType: "sttnwrkngreglang"),
                ),
              ),
            );
          },
      ...
      Expanded(
          ...
          child: Container(
              child: TextFormField(
                  ...
                  maxLines: 5,
                  onChanged: (val) {
                    sttnwrkngrulemod.rmrks1 = val;
                    //print('sttnwrkngrulemod.rmrks1:' + sttnwrkngrulemod.rmrks1);
    })),
    )),
    new IconButton(
    onPressed: _listen,
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            sttnwrkngrulemod.rmrks1= rmrkController.text;
  ...
  Future<String> _callSaveStationworkingRuleRegisterWebService(
      StationworkingRuleRegisterModel cautOrdReg) async {
    //log.d(alpa);
