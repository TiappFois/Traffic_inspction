...
import 'package:ti/commonutils/ti_utilities.dart';
import 'package:ti/model/SttnInspModels/point_cross_joint_inspModel.dart';
import 'package:ti/commonutils/logger.dart';
import 'package:speech_to_text/speech_to_text.dart' as stt;

class PointCrossingJointInspectRegister extends StatefulWidget {
  @override
  _PointCrossingJointInspectRegisterState createState() =>
      _PointCrossingJointInspectRegisterState();
}

class _PointCrossingJointInspectRegisterState
    extends State<PointCrossingJointInspectRegister> {
  File imageFile = null ;
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('Point Joint');

  static PointCrossingJointInspectRegisterModel pointcrossmod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#32#')) {

      try {
        print("Point Cross data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringPoint Cross: = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getPointCrosReg';
        ...
        if (response.statusCode == 200) {
          ...
          pointcrossmod = await new PointCrossingJointInspectRegisterModel(
              SttnInspDtlsList.getPointCrossList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
          ...
          for (int i = 0; i < 6; i++) whyNocontroller.add(TextEditingController());
      ...
      showText = await [false,false,false,false,false,false];

      pointcrossmod = await new PointCrossingJointInspectRegisterModel(
          SttnInspDtlsList.getPointCrossList(), ['SELECT','SELECT','SELECT','SELECT','SELECT','SELECT'],
          ['','','','','',''],
          '');
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          pointcrossmod.ynList1[0] +
          pointcrossmod.ynList1[1] +
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        ...
        appBar: TiUtilities.tiAppBar(
            context, "Point and Crossing Joint inspection Register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Miscellaneous"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < pointcrossmod.ynList1.length ; i++){
                if(pointcrossmod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  pointcrossmod.ynList1[i] = '';
              ...
              if(selectedCount == 0){
                ...
                pointcrossmod.base64Image = base64Image;

                pointcrossmod.inspID = TiUtilities.inspmstr.inspid;

                if (_formKey.currentState.validate()) {
                  TiUtilities.callSttnInspEntryWebService(
                      context, json.encode(pointcrossmod.toJson()),
                      "savePointCrosReg").then((res) {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: pointcrossmod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getPointCrossListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: pointcrossmod.sList1,
                      ynlst: pointcrossmod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: pointcrossmod.whyNoRsn,
                      regiType: "pointCrosslang"),
                ),
              ),
            );
          },
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            pointcrossmod.rmrks1= rmrkController.text;
  ...
  Future<String> _callSavePointCrossingJointInspectRegisterWebService(
      PointCrossingJointInspectRegisterModel cautOrdReg) async {
    //log.d(alpa);
