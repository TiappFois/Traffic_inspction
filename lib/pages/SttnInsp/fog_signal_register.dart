...
import 'package:ti/model/SttnInspModels/fog_signal_regModel.dart';
...
class FogSignalRegister extends StatefulWidget {
  @override
  _FogSignalRegisterState createState() => _FogSignalRegisterState();
}

class _FogSignalRegisterState extends State<FogSignalRegister> {
  File imageFile = null ;
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('FogSignaltReg');

  static FogSignalRegisterModel fogSignalRegMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#15#')) {

      try {
        print("FogSignal data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringFogSignal = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getFogSgnlrRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          fogSignalRegMod = await new FogSignalRegisterModel(
              SttnInspDtlsList.getfogSgnlList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Fog Signal reg");

      showText = await [false, false, false, false, false];

      fogSignalRegMod = await new FogSignalRegisterModel(
          SttnInspDtlsList.getfogSgnlList(), ['SELECT', 'SELECT','SELECT', 'SELECT','SELECT'],
          ['', '','', '',''],
          '');
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          fogSignalRegMod.ynList1[0] +
          fogSignalRegMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Fog Signal Register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/dsl_dttn_reg"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < fogSignalRegMod.ynList1.length ; i++){
                if(fogSignalRegMod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  fogSignalRegMod.ynList1[i] = '';
            ...
            fogSignalRegMod.base64Image = base64Image;

            fogSignalRegMod.inspID = TiUtilities.inspmstr.inspid;

            if (_formKey.currentState.validate()) {
              TiUtilities.callSttnInspEntryWebService(context, json.encode(fogSignalRegMod.toJson()), "saveFogSgnlrRgtr").then((res) {
                if (res == 'Record Successfully Saved.') {
                  print('Record Successfully Saved.');
                  TiUtilities.showOKDialog(context, "Success!!")
                      .then((res1) {
                    Navigator.pushNamed(context, '/dsl_dttn_reg');
                  });
                } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: fogSignalRegMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getfogSgnlListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: fogSignalRegMod.sList1,
                      ynlst: fogSignalRegMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: fogSignalRegMod.whyNoRsn,
                      regiType: "fogsignalreglang"),
                ),
              ),
            );
          },
      ...
      Expanded(
          ...
          child: Container(
              child: TextFormField(
                  ...
                  maxLines: 5,
                  onChanged: (val) {
                    fogSignalRegMod.rmrks1 = val;
                    //print('fogSignalRegMod.rmrks1:' + fogSignalRegMod.rmrks1);
    })),
    )),
    new IconButton(
    onPressed: _listen,
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            fogSignalRegMod.rmrks1= rmrkController.text;
  ...
  Future<String> _callSaveFogSignalRegisterWebService(
      FogSignalRegisterModel cautOrdReg) async {
    //log.d(alpa);
