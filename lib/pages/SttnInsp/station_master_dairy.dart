...
import 'dart:convert';
import 'package:ti/model/SttnInspModels/station_diary_model.dart';
import 'package:flutter/material.dart';
import 'package:ti/commonutils/ti_utilities.dart';
import 'package:ti/commonutils/logger.dart';


class StationMasterDiary extends StatefulWidget {
  @override
  _StationMasterDiaryState createState() => _StationMasterDiaryState();
}

class _StationMasterDiaryState extends State<StationMasterDiary> {
  File imageFile = null ;
  ...
  static StationMasterDiaryModel stationDarymod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#22#')) {

      try {
        print("Station mstr diary Reg data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringSttnMasterdiary = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getSttnMstrDiary';
        ...
        if (response.statusCode == 200) {
          ...
          stationDarymod = await new StationMasterDiaryModel(
              SttnInspDtlsList.getSttnMasterList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Sttn Diary reg");

      showText = await [false, false, false, false, false];

      stationDarymod = await new StationMasterDiaryModel(
          SttnInspDtlsList.getSttnMasterList(), ['SELECT', 'SELECT','SELECT', 'SELECT','SELECT'],
          ['', '','', '',''],
          '');
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Station Master Diary"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Failure_Memo_book"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < stationDarymod.ynList1.length ; i++){
                if(stationDarymod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  stationDarymod.ynList1[i] = '';
              ...
              if(selectedCount == 0){
                ...
                stationDarymod.base64Image = base64Image;

                stationDarymod.inspID = TiUtilities.inspmstr.inspid;

                if (_formKey.currentState.validate()) {
                  TiUtilities.callSttnInspEntryWebService(
                      context, json.encode(stationDarymod.toJson()),
                      "saveSttnMstrDiary").then((res) {
                    if (res == 'Record Successfully Saved.') {
                      print('Record Successfully Saved.');
                      TiUtilities.showOKDialog(context, "Success!!")
                          .then((res1) {
                        Navigator.pushNamed(context, '/Failure_Memo_book');
                      });
                    } else {
        ...
        ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              ...
              child: Tooltip(
                message: SttnInspDtlsList.getSttnMasterListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      ...
                      whyNolst: stationDarymod.whyNoRsn,
                      regiType: "stationDarylang"),
                ),
              ),
            );
          },
  ...
  Future<String> _callSaveStationMasterDiaryWebService(
      StationMasterDiaryModel cautOrdReg) async {
    //log.d(alpa);
