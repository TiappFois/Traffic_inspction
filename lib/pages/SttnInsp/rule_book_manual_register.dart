...
import 'package:ti/commonutils/ti_utilities.dart';
import 'package:ti/model/SttnInspModels/rule_book_Model.dart';
import 'package:ti/commonutils/logger.dart';


class RuleBookManual extends StatefulWidget {
  @override
  _RuleBookManualState createState() => _RuleBookManualState();
}
...
class _RuleBookManualState extends State<RuleBookManual> {
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('RuleBook');

  static RuleBookManualModel ruleBookMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#25#')) {

      try {
        print("Rule Book Reg data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringRule Book = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getRuleBookRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          ruleBookMod = await new RuleBookManualModel(
              SttnInspDtlsList.getRuleBookList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          ruleBookMod.ynList1[0] +
          ruleBookMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Rule Book Manual"),
        body: Builder(
          builder: (context)  {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Safety_cir_Bulletin"),
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: ruleBookMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getRuleBookListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: ruleBookMod.sList1,
                      ynlst: ruleBookMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: ruleBookMod.whyNoRsn,
                      regiType: "rulebooklang"),
                ),
              ),
            );
          },
