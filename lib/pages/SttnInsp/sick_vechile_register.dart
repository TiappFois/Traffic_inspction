...
class _SickVechileRegisterState extends State<SickVechileRegister> {
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('SickVehicletReg');

  static SickVechileRegisterModel sickVehicleRegMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#18#')) {

      try {
        print("SickVehicle data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringSickVehicle = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getSickVchlRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          sickVehicleRegMod = await new SickVechileRegisterModel(
              SttnInspDtlsList.getSickVehicleList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Sick Vehicle reg");

      showText = await [false, false, false];

      sickVehicleRegMod = await new SickVechileRegisterModel(
          SttnInspDtlsList.getSickVehicleList(), ['SELECT', 'SELECT','SELECT'],
          ['', '',''],
          '');
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          sickVehicleRegMod.ynList1[0] +
          sickVehicleRegMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/emrg_cross_over_reg"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < sickVehicleRegMod.ynList1.length ; i++){
                if(sickVehicleRegMod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  sickVehicleRegMod.ynList1[i] = '';
              ...
              if(selectedCount == 0){
                ...
                sickVehicleRegMod.base64Image = base64Image;

                sickVehicleRegMod.inspID = TiUtilities.inspmstr.inspid;

                if (_formKey.currentState.validate()) {
                  TiUtilities.callSttnInspEntryWebService(
                      context, json.encode(sickVehicleRegMod.toJson()),
                      "saveSickVchlRgtr").then((res) {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: sickVehicleRegMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getSickVehicleListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: sickVehicleRegMod.sList1,
                      ynlst: sickVehicleRegMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: sickVehicleRegMod.whyNoRsn,
                      regiType: "sickvehreglang"),
                ),
              ),
            );
          },
      ...
      Expanded(
          ...
          child: Container(
              child: TextFormField(
                  ...
                  maxLines: 5,
                  onChanged: (val) {
                    sickVehicleRegMod.rmrks1 = val;
                    //print('sickVehicleRegMod.rmrks1:' + sickVehicleRegMod.rmrks1);
    })),
    )),
    new IconButton(
    onPressed: _listen,
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            sickVehicleRegMod.rmrks1= rmrkController.text;
