...
class _MiscellaneousState extends State<Miscellaneous> {
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('Miscelleaneous');

  static MiscellaneousModel MiscellMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#33#')) {

      try {
        print("Miscellanous data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringPoint Cross: = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getMiscellReg';
        ...
        if (response.statusCode == 200) {
          ...
          MiscellMod = await new MiscellaneousModel(
              SttnInspDtlsList.getMiscList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          MiscellMod.ynList1[0] +
          MiscellMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Miscellaneous"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            MiscellMod.base64Image = base64Image;

            MiscellMod.inspID = TiUtilities.inspmstr.inspid;

            if (_formKey.currentState.validate()) {
              TiUtilities.callSttnInspEntryWebService(context, json.encode(MiscellMod.toJson()), "saveMiscellReg").then((res) {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: MiscellMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getmiscListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: MiscellMod.sList1,
                      ynlst: MiscellMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: MiscellMod.whyNoRsn,
                      regiType: "Miscelllang"),
                ),
              ),
            );
          },
  ...
  Future<String> _callSaveMiscellaneousWebService(
      MiscellaneousModel cautOrdReg) async {
    //log.d(alpa);
