...
import 'package:ti/model/SttnInspModels/staff_grievence_register_model.dart';
import 'Trn_Sgnl_Failure.dart';
import 'package:flutter/material.dart';
import 'package:ti/commonutils/ti_utilities.dart';
import 'package:ti/commonutils/logger.dart';

class StaffGreivence extends StatefulWidget {
  @override
  _StaffGreivenceState createState() => _StaffGreivenceState();
}

class _StaffGreivenceState extends State<StaffGreivence> {
  File imageFile = null ;
  ...
  static StaffGreivenceModel staffgrvMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#13#')) {

      try {
        print("taff Griev data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringStaffgriev = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getStffGrvncRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          staffgrvMod = await new StaffGreivenceModel(
              SttnInspDtlsList.getStaffGrievList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Staff Griev reg");

      showText = await [false, false, false];

      staffgrvMod = await new StaffGreivenceModel(
          SttnInspDtlsList.getStaffGrievList(), ['SELECT', 'SELECT','SELECT'],
          ['', '',''],
          '');
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          staffgrvMod.ynList1[0] +
          staffgrvMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Staff Grievance  Register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
            return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/axle_counter_reg"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < staffgrvMod.ynList1.length ; i++){
                if(staffgrvMod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  staffgrvMod.ynList1[i] = '';
              ...
              if(selectedCount == 0){
                ...
                staffgrvMod.base64Image = base64Image;

                staffgrvMod.inspID = TiUtilities.inspmstr.inspid;

                if (_formKey.currentState.validate()) {
                  TiUtilities.callSttnInspEntryWebService(
                      context, json.encode(staffgrvMod.toJson()),
                      "saveStffGrvncRgtr").then((res) {
                    if (res == 'Record Successfully Saved.') {
                      print('Record Successfully Saved.');
                      TiUtilities.showOKDialog(context, "Success!!")
                          .then((res1) {
                        Navigator.pushNamed(context, '/axle_counter_reg');
                      });
                    } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: staffgrvMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getStaffGrievListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: staffgrvMod.sList1,
                      ynlst: staffgrvMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: staffgrvMod.whyNoRsn,
                      regiType: "staffgrievlang"),
                ),
              ),
            );
          },
      ...
      Expanded(
          ...
          child: Container(
              child: TextFormField(
                  ...
                  maxLines: 5,
                  onChanged: (val) {
                    staffgrvMod.rmrks1 = val;
                    //print('staffgrvMod.rmrks1:' + staffgrvMod.rmrks1);
    })),
    )),
    new IconButton(
    onPressed: _listen,
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            staffgrvMod.rmrks1= rmrkController.text;
  ...
  Future<String> _callSaveStaffGreivenceWebService(
      StaffGreivenceModel cautOrdReg) async {
    //log.d(alpa);
