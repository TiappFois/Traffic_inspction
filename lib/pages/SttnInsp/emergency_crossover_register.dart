...
import 'package:ti/model/SttnInspModels/emergency_cross_over_reg_model.dart';
...
class EmergencyCrossoverRegister extends StatefulWidget {
  @override
  _EmergencyCrossoverRegisterState createState() =>
      _EmergencyCrossoverRegisterState();
}
...
class _EmergencyCrossoverRegisterState
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('EmerCrosReg');

  static EmergencyCrossoverRegisterModel emerCrosMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#19#')) {
      ...
      try {
        ...
        log.d("urlInputStringEmerCros = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getEmrGcroSoverRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          emerCrosMod = await new EmergencyCrossoverRegisterModel(
              SttnInspDtlsList.getEmerCrossList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Emer Cross reg");
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          emerCrosMod.ynList1[0] +
          emerCrosMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Emergency Cross-over Register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/attendance_reg"),
        ...
        floatingActionButton: FloatingActionButton.extended(
          ...
          onPressed: () async {
            ...
            emerCrosMod.base64Image = base64Image;

            if (_formKey.currentState.validate()) {
              TiUtilities.callSttnInspEntryWebService(context, json.encode(emerCrosMod.toJson()), "saveEmrGcroSoverRgtr").then((res) {
                if (res == 'Record Successfully Saved.') {
                  print('Record Successfully Saved.');
                  TiUtilities.showOKDialog(context, "Success!!")
                      .then((res1) {
                    Navigator.pushNamed(context, '/attendance_reg');
                  });
                } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: emerCrosMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getEmerCrossListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: emerCrosMod.sList1,
                      ynlst: emerCrosMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: emerCrosMod.whyNoRsn,
                      regiType: "emercrosreglang"),
                ),
              ),
            );
          },
