...
import 'package:ti/model/SttnInspModels/disel_detention_registerModel.dart';
...
class DieselDetention extends StatefulWidget {
  @override
  _DieselDetentionState createState() => _DieselDetentionState();
}
...
class _DieselDetentionState extends State<DieselDetention> {
  ...
  static DieselDetentionModel dsldetnModel;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#16#')) {

      try {
        print("DieselDtn data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringDieseldtn = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getDesldTtnRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          dsldetnModel = await new DieselDetentionModel(
              SttnInspDtlsList.getDieselList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Diesel dtnn reg");
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          dsldetnModel.ynList1[0] +
          dsldetnModel.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Diesel Detention Register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Stbl_load_reg"),
        ...
        floatingActionButton: FloatingActionButton.extended(
          ...
          onPressed: () async {
            ...
            dsldetnModel.base64Image = base64Image;

            dsldetnModel.inspID = TiUtilities.inspmstr.inspid;

            if (_formKey.currentState.validate()) {
              TiUtilities.callSttnInspEntryWebService(context, json.encode(dsldetnModel.toJson()), "saveDesldTtnRgtr").then((res) {
                if (res == 'Record Successfully Saved.') {
                  print('Record Successfully Saved.');
                  TiUtilities.showOKDialog(context, "Success!!")
                      .then((res1) {
                    Navigator.pushNamed(context, '/Stbl_load_reg');
                  });
                } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: dsldetnModel.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getDieselListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: dsldetnModel.sList1,
                      ynlst: dsldetnModel.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: dsldetnModel.whyNoRsn,
                      regiType: "diesdtnreglang"),
                ),
              ),
            );
          },
