...
class _PublicComplaintBoxState extends State<PublicComplaintBox> {
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('Public Complaint');

  static PublicComplaintBoxModel publicComplMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#28#')) {

      try {
        print("Public Complaint data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringPublic Compl : = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getpublicComplreg';
        ...
        if (response.statusCode == 200) {
          ...
          publicComplMod = await new PublicComplaintBoxModel(
              SttnInspDtlsList.getPublicCompList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Public Complaint");
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          publicComplMod.ynList1[0] +
          publicComplMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Panel_countr_Reg"),
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: publicComplMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getPublicCompListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: publicComplMod.sList1,
                      ynlst: publicComplMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: publicComplMod.whyNoRsn,
                      regiType: "publicComplang"),
                ),
              ),
            );
          },
