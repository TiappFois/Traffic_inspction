...
class EssentialSafetyEquiments extends StatefulWidget {
  @override
  _EssentialSafetyEquimentsState createState() =>
      _EssentialSafetyEquimentsState();
}
...
class _EssentialSafetyEquimentsState extends State<EssentialSafetyEquiments> {
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('EssenEquipReg');

  static EssentialSafetyEquimentsModel esseneqipMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#24#')) {

      try {
        print("Essen Equip Reg data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringEssenEquip = " + urlInputString);
        var url= TiConstants.webServiceUrl +'v';
        ...
        if (response.statusCode == 200) {
          ...
          esseneqipMod = await new EssentialSafetyEquimentsModel(
              SttnInspDtlsList.getEssnSafeList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          esseneqipMod.ynList1[0] +
          esseneqipMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Essential Safety Equipments"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Rule_book_Manul"),
        ...
        floatingActionButton: FloatingActionButton.extended(
          ...
          onPressed: () async {
            ...
            esseneqipMod.base64Image = base64Image;

            esseneqipMod.inspID = TiUtilities.inspmstr.inspid;

            if (_formKey.currentState.validate()) {
              TiUtilities.callSttnInspEntryWebService(context, json.encode(esseneqipMod.toJson()), "saveEssnSftyEqupRgtr").then((res) {
                if (res == 'Record Successfully Saved.') {
                  print('Record Successfully Saved.');
                  TiUtilities.showOKDialog(context, "Success!!")
                      .then((res1) {
                    Navigator.pushNamed(context, '/Rule_book_Manul');
                  });
                } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: esseneqipMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getEssnSafeListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: esseneqipMod.sList1,
                      ynlst: esseneqipMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: esseneqipMod.whyNoRsn,
                      regiType: "esseneqiplang"),
                ),
              ),
            );
          },
  ...
  Future<String> _callSaveEssentialSafetyEquimentsWebService(
      EssentialSafetyEquimentsModel cautOrdReg) async {
    //log.d(alpa);
