...
import 'package:ti/model/SttnInspModels/axle_counter_register_model.dart';
...
class AxleCounterRegister extends StatefulWidget {
  @override
  _AxleCounterRegisterState createState() => _AxleCounterRegisterState();
}
...
class _AxleCounterRegisterState extends State<AxleCounterRegister> {
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('AxleCountertReg');

  static AxleCounterRegisterModel axleCounterMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#14#')) {

      try {
        print("AxleCounter data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringSgnlFailure = " + urlInputString);

        var url= TiConstants.webServiceUrl +'getAxlecntrRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          axleCounterMod  = await new AxleCounterRegisterModel(
              SttnInspDtlsList.getAxleList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Axle Counter reg");
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          axleCounterMod.ynList1[0] +
          axleCounterMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Axle Counter Register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/fog_reg_griv"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            axleCounterMod.base64Image = base64Image;

            if (_formKey.currentState.validate()) {
              TiUtilities.callSttnInspEntryWebService(context, json.encode(axleCounterMod.toJson()), "saveAxlecntrRgtr").then((res) {
                if (res == 'Record Successfully Saved.') {
                  print('Record Successfully Saved.');
                  TiUtilities.showOKDialog(context, "Success!!")
                      .then((res1) {
                    Navigator.pushNamed(context, '/fog_reg_griv');
                  });
                } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: axleCounterMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getAxleListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: axleCounterMod.sList1,
                      ynlst: axleCounterMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: axleCounterMod.whyNoRsn,
                      regiType: "axlecounterlang"),
                ),
              ),
            );
          },
