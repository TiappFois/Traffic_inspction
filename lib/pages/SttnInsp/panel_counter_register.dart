...
class PanelCounterRegister extends StatefulWidget {
  @override
  _PanelCounterRegisterState createState() => _PanelCounterRegisterState();
}
...
class _PanelCounterRegisterState extends State<PanelCounterRegister> {
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('Panel Counter');

  static PanelCounterRegisterModel panelcounterregMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#29#')) {

      try {
        print("Panel Counter data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringPanel Counter : = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getpanelcounterreg';
        ...
        if (response.statusCode == 200) {
          ...
          panelcounterregMod = await new PanelCounterRegisterModel(
              SttnInspDtlsList.getPanelCounterList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, " Panel Counter Register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Power_traf_blck_reg"),
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: panelcounterregMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getPanelCounterListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: panelcounterregMod.sList1,
                      ynlst: panelcounterregMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: panelcounterregMod.whyNoRsn,
                      regiType: "panelcounterlang"),
                ),
              ),
            );
          },
  ...
  Future<String> _callSavePanelCounterRegisterWebService(
      PanelCounterRegisterModel cautOrdReg) async {
    //log.d(alpa);
