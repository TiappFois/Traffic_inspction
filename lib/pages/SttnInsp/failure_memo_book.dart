...
class _FailureMemoBookState extends State<FailureMemoBook> {
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('FailureMemoReg');

  static FailureMemoBookModel failurememomod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#23#')) {

      try {
        print("Failure memo Reg data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringFailureMemo= " + urlInputString);
        var url= TiConstants.webServiceUrl +'getFailureMenoReg';
        ...
        if (response.statusCode == 200) {
          ...
          failurememomod = await new FailureMemoBookModel(
              SttnInspDtlsList.getFailMemoList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Failure Memo reg");

      showText = await [false, false, false, false, false];

      failurememomod = await new FailureMemoBookModel(
          SttnInspDtlsList.getFailMemoList(), ['SELECT', 'SELECT','SELECT', 'SELECT','SELECT'],
          ['', '','', '',''],
          '');
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          failurememomod.ynList1[0] +
          failurememomod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Essen_Safet_Equip"),

        floatingActionButton: FloatingActionButton.extended(

            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < failurememomod.ynList1.length ; i++){
                if(failurememomod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  failurememomod.ynList1[i] = '';
            ...
            failurememomod.base64Image = base64Image;

            failurememomod.inspID = TiUtilities.inspmstr.inspid;

            if (_formKey.currentState.validate()) {
              TiUtilities.callSttnInspEntryWebService(context, json.encode(failurememomod.toJson()), "saveFailureMenoReg").then((res) {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: failurememomod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding: const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getFailMemoListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: failurememomod.sList1,
                      ynlst: failurememomod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: failurememomod.whyNoRsn,
                      regiType: "failurememolang"),
                ),
              ),
            );
          },
      ...
      Expanded(
          ...
          child: Container(
              child: TextFormField(
                  ...
                  maxLines: 5,
                  onChanged: (val) {
                    failurememomod.rmrks1 = val;
                    //print('failurememomod.rmrks1:' + failurememomod.rmrks1);
    })),
    )),
    new IconButton(
    onPressed: _listen,
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            failurememomod.rmrks1= rmrkController.text;
