...
import 'package:ti/model/SttnInspModels/attendance_register_model.dart';
...
class _AttendanceRegisterState extends State<AttendanceRegister> {
  ...
  static AttendanceRegisterModel attndregMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#20#')) {
      ...
      try {
        ...
        var url= TiConstants.webServiceUrl +'getAttendRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          attndregMod = await new AttendanceRegisterModel(
              SttnInspDtlsList.getAttndList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
          ...
          imageFile = File("$dir/" + DateTime.now().millisecondsSinceEpoch.toString() + ".png");
   ...
   callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          attndregMod.ynList1[0] +
          attndregMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Attendance  Register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Sttn_wrkg_rule_reg"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            attndregMod.base64Image = base64Image;

            if (_formKey.currentState.validate()) {
              TiUtilities.callSttnInspEntryWebService(context, json.encode(attndregMod.toJson()), "saveAttendRgtr").then((res) {
                if (res == 'Record Successfully Saved.') {
                  print('Record Successfully Saved.');
                  TiUtilities.showOKDialog(context, "Success!!")
                      .then((res1) {
                    Navigator.pushNamed(context, '/Sttn_wrkg_rule_reg');
                  });
                } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: attndregMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getAttndListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: attndregMod.sList1,
                      ynlst: attndregMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: attndregMod.whyNoRsn,
                      regiType: "attndreglang"),
                ),
              ),
            );
          },
