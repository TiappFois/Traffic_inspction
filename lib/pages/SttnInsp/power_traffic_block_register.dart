...
import 'package:ti/commonutils/ti_utilities.dart';
import 'package:ti/model/SttnInspModels/power_traffic_block_model%20.dart';
import 'package:ti/commonutils/logger.dart';
import 'package:speech_to_text/speech_to_text.dart' as stt;

class PowerTrafficBlockRegister extends StatefulWidget {
  @override
  _PowerTrafficBlockRegisterState createState() =>
      _PowerTrafficBlockRegisterState();
}

class _PowerTrafficBlockRegisterState extends State<PowerTrafficBlockRegister> {
  File imageFile = null ;
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('Power Traffic');

  static PowerTrafficBlockRegisterModel powerTrafficMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#30#')) {

      try {
        print("Power Traffic data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringPower Traffic: = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getpowerTrfcBlkreg';
        ...
        if (response.statusCode == 200) {
          ...
          powerTrafficMod = await new PowerTrafficBlockRegisterModel(
              SttnInspDtlsList.getPowerTrafficList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Panel Counter");

      showText = await [false];

      powerTrafficMod = await new PowerTrafficBlockRegisterModel(
          SttnInspDtlsList.getPowerTrafficList(), ['SELECT'],
          [''],
          '');
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar:
            TiUtilities.tiAppBar(context, " Power & Traffic Block register"),
        body: Builder(
          builder: (context) {
            if (gettingData == 0) {
              return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/Priv_Number_Book"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < powerTrafficMod.ynList1.length ; i++){
                if(powerTrafficMod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  powerTrafficMod.ynList1[i] = '';
              ...
              if(selectedCount == 0){
                ...
                powerTrafficMod.base64Image = base64Image;

                powerTrafficMod.inspID = TiUtilities.inspmstr.inspid;

                if (_formKey.currentState.validate()) {
                  TiUtilities.callSttnInspEntryWebService(
                      context, json.encode(powerTrafficMod.toJson()),
                      "savepowerTrfcBlkreg").then((res) {
                    if (res == 'Record Successfully Saved.') {
                      print('Record Successfully Saved.');
                      TiUtilities.showOKDialog(context, "Success!!")
                          .then((res1) {
                        Navigator.pushNamed(context, '/Priv_Number_Book');
                      });
                    } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: powerTrafficMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getPowerTrafficListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: powerTrafficMod.sList1,
                      ynlst: powerTrafficMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: powerTrafficMod.whyNoRsn,
                      regiType: "powertrafficlang"),
                ),
              ),
            );
          },
      ...
      Expanded(
          ...
          child: Container(
              child: TextFormField(
                  ...
                  maxLines: 5,
                  onChanged: (val) {
                    powerTrafficMod.rmrks1 = val;
                    //print('powerTrafficMod.rmrks1:' + powerTrafficMod.rmrks1);
    })),
    )),
    new IconButton(
    onPressed: _listen,
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            powerTrafficMod.rmrks1= rmrkController.text;
  ...
  Future<String> _callSavePowerTrafficBlockRegisterWebService(
      PowerTrafficBlockRegisterModel cautOrdReg) async {
    //log.d(alpa);
