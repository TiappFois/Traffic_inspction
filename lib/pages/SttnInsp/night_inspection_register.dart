...
import 'package:ti/model/SttnInspModels/night_inspection_Model.dart';
import 'package:ti/commonutils/logger.dart';
import 'package:speech_to_text/speech_to_text.dart' as stt;
class NightInspectionRegister extends StatefulWidget {
  @override
  _NightInspectionRegisterState createState() =>
      _NightInspectionRegisterState();
}

class _NightInspectionRegisterState extends State<NightInspectionRegister> {
  File imageFile = null ;
  ...
  List<bool> showText;
  int gettingData = 0;
  static final log = getLogger('NightInspReg');

  static NightInspectionRegisterModel nightInspMod;
  ...
  Future<void> getData() async {
    ...
    if (TiUtilities.inspmstr.cinsplist.contains('#10#')) {

      try {
        print("Night Insp data");

        String urlInputString = TiUtilities.inspmstr.inspid;

        log.d("urlInputStringNihtInsp = " + urlInputString);
        var url= TiConstants.webServiceUrl +'getNghtInspcRgtr';
        ...
        if (response.statusCode == 200) {
          ...
          nightInspMod = await new NightInspectionRegisterModel(
              SttnInspDtlsList.getNghtList(), jsonResult['ynList1'],
              jsonResult['whyNoRsn'],
      ...
      print ("In else of Night Insp reg");

      showText = await [false, false, false, false, false];

      nightInspMod = await new NightInspectionRegisterModel(
          SttnInspDtlsList.getNghtList(), ['SELECT', 'SELECT','SELECT', 'SELECT','SELECT'],
          ['', '','', '',''],
          '');
  ...
  callbackDropDown(callBackController, selected, show) {
    setState(() {
      ...
      print('callBackController:' +
          nightInspMod.ynList1[0] +
          nightInspMod.ynList1[1] +
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: new Scaffold(
        key: _scaffoldkey,
        appBar: TiUtilities.tiAppBar(context, "Night Inspection Register"),
        body: Builder(
          builder: (context) {
          if (gettingData == 0) {
          return Container(
        ...
        bottomNavigationBar: TiUtilities.tiBottomNaviBar(context, "/over_time_reg"),
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () async {
              int selectedCount = 0 ;
              for(var i = 0 ; i < nightInspMod.ynList1.length ; i++){
                if(nightInspMod.ynList1[i] != "SELECT"){
                  selectedCount = 1 ;
                }
                else
                  nightInspMod.ynList1[i] = '';
              ...
              if(selectedCount == 0){
                ...
                nightInspMod.base64Image = base64Image;

                nightInspMod.inspID = TiUtilities.inspmstr.inspid;

                if (_formKey.currentState.validate()) {
                  TiUtilities.callSttnInspEntryWebService(
                      context, json.encode(nightInspMod.toJson()),
                      "saveNightInapRgtrs").then((res) {
                    if (res == 'Record Successfully Saved.') {
                      print('Record Successfully Saved.');
                      TiUtilities.showOKDialog(context, "Success!!")
                          .then((res1) {
                        Navigator.pushNamed(context, '/over_time_reg');
                      });
                    } else {
    ...
    return Column(
      children: [
        ListView.builder(
          ...
          itemCount: nightInspMod.sList1.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding:
                  const EdgeInsets.symmetric(vertical: 5.0, horizontal: 4.0),
              child: Tooltip(
                message: SttnInspDtlsList.getNghtListDtls()[index],
                child: Card(
                  child: dropdowncls(
                      dropdowntextcallback: callbackDropDown,
                      slst: nightInspMod.sList1,
                      ynlst: nightInspMod.ynList1,
                      index: index,
                      selectedItem: selectedItemValue,
                      showTextField: showText,
                      whyNoCont: whyNocontroller,
                      whyNolst: nightInspMod.whyNoRsn,
                      regiType: "nightinsplang"),
                ),
              ),
            );
          },
      ...
      Expanded(
          ...
          child: Container(
              child: TextFormField(
                  ...
                  maxLines: 5,
                  onChanged: (val) {
                    nightInspMod.rmrks1 = val;
                    //print('nightInspMod.rmrks1:' + nightInspMod.rmrks1);
    })),
    )),
    new IconButton(
    onPressed: _listen,
      ...
      if (available) {
        ...
        _speech.listen(
          onResult: (val) => setState(() {
            ...
            nightInspMod.rmrks1= rmrkController.text;
  ...
  Future<String> _callSaveNightInspectionRegisterWebService(
      NightInspectionRegisterModel cautOrdReg) async {
    //log.d(alpa);
